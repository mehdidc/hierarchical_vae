...
class VAE(nn.Module):
    def __init__(
        ...
        wl = w // 2 ** (nb_blocks + 1)
        self.latent = nn.Sequential(nn.Linear(nf * wl * wl, latent_size * 2))
        self.post_latent = nn.Sequential(nn.Linear(latent_size, nf * wl * wl))
        ...
        for _ in range(nb_blocks):
            ...
            nf = nf // 2
        layers.append(nn.ConvTranspose2d(nf, nc, 4, 2, 1, bias=False))
